...
def sub_menu(list_category, rede, list_treinamento=None):
    ...
    if not p['atual'] and list_treinamento.count() > 0:
        p['atual'] = list_treinamento[0].category
...
def videos_relacionados(list_video, rede):
...
def get_icone(rede, tipo):
    ...
    return '%s%s/images/icone_m.gif' % (settings.STATIC_URL, tipo)
...
def is_not_video(obj):
    ...
    elear = Elearning.objects.filter( Q(visible = True), Q(treinamento = obj), Q(not_video = True) ).count() > 0
