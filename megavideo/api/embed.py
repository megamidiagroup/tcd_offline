...
def embed_js(request):
    ...
    render_to_html = request.REQUEST.get('render_to_html' , 0)
    p['jquery']    = request.REQUEST.get('jquery'         , 'true')
    ...
    p['static']   = settings.MEGAVIDEO_CONF['base_url'] + 'static/portal/'

    try:
        idContent  = Video().deserialize( p['idContent'] )
        video      = Video.objects.using('megavideo').get(id = int(idContent))
        p['video'] = video
    except:
        p['video'] = []
    ...
    if p['jquery'] == 'true':
        js  += render_to_string('api/jquery-1.8.2.min.js'            , p, context_instance=RequestContext(request))
        js  += render_to_string('api/mediaelement-and-player.min.js' , p, context_instance=RequestContext(request))
        #p['me_css'] = render_to_string('api/mediaelementplayer.min.css'     , p, context_instance=RequestContext(request))

    p['html5']  = render_to_string('api/html5.html', p, context_instance=RequestContext(request))

    text        = render_to_string('api/embed.html', p, context_instance=RequestContext(request))

    if render_to_html:
        text = '<script type=\'text/javascript\'>%s</script>%s' % (js, text)
