...
(function($) {
	...
	$.formValidator = function (options) {
		...
		$(merged_options.scope+' .req-email, '+merged_options.scope+' .req-string, '+merged_options.scope+' .req-same, '+merged_options.scope+' .req-both, '+merged_options.scope+' .req-numeric, '+merged_options.scope+' .req-date, '+merged_options.scope+' .req-min, ' + merged_options.scope+' .req-checked').each(function() {
	...
	$.formValidator.validate = function(obj,opts) {
		...
		// REQUIRED CHECKED VALIDATION
		if (obj.hasClass('req-checked')) {
			var valChecked = obj.attr('checked');
			tmpresult = valChecked;
	...
	$.formValidator.defaults = {
		...
		errorMsg		: 	{
								...
								reqCheck	    :	'É necessário confirmar o termo de cadastro ',
