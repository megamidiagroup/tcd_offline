...
class CaptchaTextInput(MultiWidget):
    ...
    def render(self, name, value, attrs=None):
        ...
        store, created = CaptchaStore.objects.using('megavideo').get_or_create(challenge=challenge,response=response)
        key = store.hashkey
        value = [key, u'']
...
class CaptchaField(MultiValueField):
    ...
    def clean(self, value):
        ...
        try:
            store = CaptchaStore.objects.using('megavideo').get(response=response,hashkey=value[0], expiration__gt=datetime.datetime.now())
            store.delete()
        except Exception:
